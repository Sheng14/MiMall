    ...
    methods: {
        ...
        getCartList () { // 获取购物车商品数据
            axios.get('/carts').then((res) => {
                let list = res.cartProductVoList // 购物车所有商品
                this.cartTotalPrice = res.cartTotalPrice // 购物车选中商品总价
                this.cartList = list.filter(item => item.productSelected) // 购物车选中商品数组
                this.cartList.map((item) => { // 拿到选中商品件数
